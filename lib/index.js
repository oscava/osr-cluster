...
var Cluster = Emitter.extends({
	$:function( name , config){
		...
		this.client.on("message",function( channel, datas ){
			try{
				...
				_this.emit("2::master",{ method: "error", datas: datas, error: e.message });
			}
		});
	...
	status:function( auth ){
		if(auth == "cavacn"){
			var _p = [];
			for(var key in this.processes){
				var process = this.processes[key];
				_p.push({ name: process.name, path: process.path ,startTime: process.startTime, config: process.config });
			}
